...
| E60 Combate empolgante (refúgio + combo + golpes variados) | ✅ três refinos pós-playtest: (1) **vila é refúgio** — inimigos não perseguem quem está em zona segura (fim do "sobe de nível parado na cidade"; provado por repro que XP é só por morte, e que os bichos assediavam o hub); (2) **combo** sobe até ×40 na contagem (empolgante) mas o dano satura em 8 stacks (+96%, balanceamento intacto), janela alargada e errar não zera mais — mantém metade + renova a graça; (3) **golpes variados** — cada swing alterna machadada/corte-esq/corte-dir/estocada, amplitude escala com o combo, e a VFX do arco cresce/brilha e alterna o lado conforme o streak. Testes `aiSafeZone`/`combo`/`combatFeel` — ADR 0183 | — |
| E57–E59 Fix de combate (direção/flash/tremor/XP) | ✅ três bugs do ataque: (E58) o **arco do golpe** saía "pro lado/pra trás" — a VFX usava outra convenção de ângulo que o modelo e o `meleeArc`; agora aponta para onde o herói olha `(sin θ, cos θ)`; (E59) a **calça piscava em todo mundo** ao acertar — materiais voxel são compartilhados por cor e o flash mexia neles in-place; agora é **clone-on-write** (flash isolado no alvo) — e o **tremor de tela** foi comedido (combo/morte comum quase não trepidam; chefe/queda seguem); (E57) "**subo de nível toda vez que ataco, até uns 4**" — repro provou que XP só vem da MORTE de inimigo (fauna excluída, sem "XP por hit"); a causa era a **curva inicial rasa** (`xpBase 38→64`), que despejava 3–4 níveis na 1ª luta. Testes `combatFeel` + canário da curva — ADR 0182 | — |
| E56 Transparência por oclusão (câmera vê o herói) | ✅ o cenário entre o jogador e a câmera fica **translúcido** (Carvalho-Mãe, casas, torres) — o herói nunca mais fica escondido, e vê-se o que está atrás. Como a câmera é ortográfica, um raio de cada jogador em direção à câmera acha os obstáculos; criaturas e folhagem instanciada não somem; clona o material 1× (sem vazar p/ compartilhados) e volta ao opaco com transição suave ao sair da frente. Vale também no interior (parede da frente some sozinha). Testes headless — ADR 0181 | — |
| E55 Porta do interior alinhada à entrada | ✅ ao entrar numa casa o jogador surgia no FUNDO da sala (parede oposta à porta). Agora surge **colado à porta de saída** (parede sul), de frente pra sala — a porta do interior corresponde à entrada. De brinde, corrigido um bug latente: `interactionSystem` passa a agir só no interativo MAIS PRÓXIMO por toque (senão entrar teleportava o jogador pra junto da saída e disparava enter+exit no mesmo quadro). Testes de regressão — ADR 0180 | — |
| E54 Afixos comportamentais + dons de dano no simulador | ✅ `runScenario({ affixes: [...] })` injeta afixos de comportamento no gear (Sedento/lifesteal, Espinhos/thorns, Ceifar) — os dons de dano já vinham por `boons`. Medido (3 comuns L10): Sedento 76%→95% de vida (sustenta), Espinhos +DPS efetivo (reflete), Instinto de Caça +dano. Fecha os 3 tipos de modificador no simulador (stat/behavior/dons). Só medição; teste trava — ADR 0179 (adendo) | — |
